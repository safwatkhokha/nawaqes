...
// src/database/auto-restore.ts
var auto_restore_exports = {};
__export(auto_restore_exports, {
  autoRestoreDB: () => autoRestoreDB
});
import fs5 from "fs";
...
async function autoRestoreDB() {
  ...
  if (fs5.existsSync(DB_FILE) && fs5.statSync(DB_FILE).size > 0) {
...
"`, { encoding: "utf-8", env: { ...process.env, HF_TOKEN }, timeout: 3e4 }).trim();
    ...
    const tempPath = path5.join(PERSISTENT_DIR2, "restore_temp.db.gz");
    execSync(`python3 -c "
...
"`, { encoding: "utf-8", env: { ...process.env, HF_TOKEN }, timeout: 12e4 });
    execSync(`gunzip -f "${tempPath}"`, { stdio: "ignore" });
    const decompressed = tempPath.replace(".gz", "");
    if (fs5.existsSync(decompressed)) {
      fs5.copyFileSync(decompressed, DB_FILE);
      fs5.unlinkSync(decompressed);
...
var PERSISTENT_DIR2, DB_FILE, HF_TOKEN, HF_BACKUP_REPO;
var init_auto_restore = __esm({
  "src/database/auto-restore.ts"() {
    "use strict";
    PERSISTENT_DIR2 = fs5.existsSync("/data") ? "/data" : path5.resolve(process.cwd(), "data");
    DB_FILE = path5.join(PERSISTENT_DIR2, "nawaqes.db");
    HF_TOKEN = process.env.HF_TOKEN || "";
    HF_BACKUP_REPO = process.env.HF_BACKUP_REPO || "safwatkhokha/nawaqes-backups";
  }
});

// src/database/backup-system.ts
var backup_system_exports = {};
__export(backup_system_exports, {
...
function createBackup(type) {
  try {
    if (!fs6.existsSync(DB_FILE2)) return;
    const timestamp = (/* @__PURE__ */ new Date()).toISOString().replace(/[:.]/g, "-").slice(0, 19);
    const dateStr = (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
    const backupPath = path6.join(BACKUP_DIR, `${type}_${timestamp}.db.gz`);
    const tempDb = path6.join(BACKUP_DIR, `temp_${timestamp}.db`);
    execSync2(`sqlite3 "${DB_FILE2}" ".backup '${tempDb}'"`, { stdio: "ignore" });
    execSync2(`gzip -f "${tempDb}"`, { stdio: "ignore" });
    fs6.renameSync(tempDb + ".gz", backupPath);
    if (HF_TOKEN2) {
      try {
...
from huggingface_hub import HfApi
api = HfApi()
api.upload_file(path_or_fileobj='${backupPath}', path_in_repo='${pathInRepo}', repo_id='${HF_BACKUP_REPO2}', repo_type='dataset', commit_message='Backup ${timestamp}')
"`, { encoding: "utf-8", env: { ...process.env, HF_TOKEN: HF_TOKEN2 }, timeout: 12e4 });
        console.log(`[BACKUP] \u2705 Uploaded: ${pathInRepo}`);
      } catch (err) {
        console.warn(`[BACKUP] Upload failed: ${err.message?.slice(0, 100)}`);
      }
    }
    const files = fs6.readdirSync(BACKUP_DIR).filter((f) => f.endsWith(".db.gz")).sort().reverse();
    files.slice(48).forEach((f) => {
      try {
        fs6.unlinkSync(path6.join(BACKUP_DIR, f));
      } catch {
      }
    });
  } catch (err) {
...
function initAutoBackup() {
  ...
  setTimeout(() => createBackup("startup"), 3e4);
  setInterval(() => createBackup("hourly"), 15 * 60 * 1e3);
  setInterval(() => createBackup("daily"), 24 * 60 * 60 * 1e3);
...
function getBackupStats() {
  const files = fs6.existsSync(BACKUP_DIR) ? fs6.readdirSync(BACKUP_DIR).filter((f) => f.endsWith(".db.gz")) : [];
  return { totalBackups: files.length, hfConfigured: !!HF_TOKEN2, hfRepo: HF_BACKUP_REPO2 };
}
var PERSISTENT_DIR3, DB_FILE2, BACKUP_DIR, HF_TOKEN2, HF_BACKUP_REPO2;
var init_backup_system = __esm({
  "src/database/backup-system.ts"() {
    "use strict";
    PERSISTENT_DIR3 = fs6.existsSync("/data") ? "/data" : path6.resolve(process.cwd(), "data");
    DB_FILE2 = path6.join(PERSISTENT_DIR3, "nawaqes.db");
    BACKUP_DIR = path6.join(PERSISTENT_DIR3, "backups");
    HF_TOKEN2 = process.env.HF_TOKEN || "";
    HF_BACKUP_REPO2 = process.env.HF_BACKUP_REPO || "safwatkhokha/nawaqes-backups";
    if (!fs6.existsSync(BACKUP_DIR)) fs6.mkdirSync(BACKUP_DIR, { recursive: true });
  }
});
...
router.post("/forgot-password", (req, res) => {
  try {
    ...
    const resetCode = crypto2.randomInt(1e5, 999999).toString();
...
var chatImageStorage = multer.diskStorage({
  ...
  filename: (_req, file, cb) => {
    ...
    cb(null, `${crypto3.randomBytes(16).toString("hex")}${ext}`);
  }
});
...
router3.post("/send", authMiddleware, (req, res) => {
  try {
    ...
    const messageId = crypto3.randomBytes(16).toString("hex").toLowerCase();
    database_default.prepare(`
...
router4.post("/admin/charging-requests/:id/approve", authMiddleware, adminMiddleware, (req, res) => {
  try {
    ...
    try {
      const wsManager2 = req.app.locals?.wsManager;
      if (wsManager2) {
        wsManager2.sendToUser(cr.user_id, { type: "wallet:updated", data: { userId: cr.user_id, amount: cr.amount } });
      }
    } catch {
    }
...
router5.get("/database-info", (req, res) => {
  try {
    ...
    try {
      ...
      const dbPath2 = path8.join(process.cwd(), "data", "nawaqes.db");
      if (fs8.existsSync(dbPath2)) {
        const stats = fs8.statSync(dbPath2);
        dbSize = stats.size;
      }
    } catch {
    }
...
router5.post("/market-promotion-requests/:id/approve", (req, res) => {
  try {
    ...
    try {
      const wsManager2 = req.app.locals.wsManager;
      if (wsManager2) wsManager2.broadcast({ type: "data:refresh", data: { reason: "market-promotion-approved" } });
    } catch {
    }
    try {
...
var videoStorage = multer2.diskStorage({
  ...
  filename: (_req, file, cb) => {
    ...
    cb(null, `vid_${crypto4.randomBytes(16).toString("hex")}${ext}`);
  }
});
...
router6.post("/market-live/link-video", authMiddleware, (req, res) => {
  try {
    ...
    const videoId = crypto4.randomBytes(16).toString("hex").toLowerCase();
...
router6.post("/promotions", authMiddleware, (req, res) => {
  try {
    ...
    const promoId = crypto4.randomBytes(16).toString("hex").toLowerCase();
  ...
  try {
    ...
    const result = suggestions.filter((u) => {
      ...
      return {
        ...
        isOnline: req.app.locals?.wsManager?.isUserOnline(u.id) || false
      };
    });
    res.json(result);
  } catch (err) {
...
router6.post("/posts/:id/share", authMiddleware, (req, res) => {
  try {
    ...
    const shareId = crypto4.randomBytes(16).toString("hex").toLowerCase();
...
router6.get("/smart-link/:postId", optionalAuth, (req, res) => {
  try {
    ...
    const visitId = crypto4.randomBytes(16).toString("hex").toLowerCase();
    try {
...
router6.post("/market-live/:videoId/comments", authMiddleware, (req, res) => {
  try {
    ...
    const commentId = crypto4.randomBytes(16).toString("hex").toLowerCase();
...
router6.get("/alerts/active", optionalAuth, (req, res) => {
  try {
    ...
    query += ` ORDER BY
      ...
      created_at DESC
    `;
    const alerts = database_default.prepare(query).all(...params);
    res.json({ alerts });
  } catch (err) {
...
router7.post("/market-live/interact", authMiddleware, (req, res) => {
  try {
    ...
    switch (interactionType) {
      case "like":
        database_default.prepare("UPDATE market_listings SET likes_count = likes_count + 1, updated_at = datetime('now') WHERE id = ?").run(videoId);
        break;
      case "save":
        ...
        break;
      case "share":
        database_default.prepare("UPDATE market_listings SET shares_count = shares_count + 1, updated_at = datetime('now') WHERE id = ?").run(videoId);
        break;
      case "view":
...
router9.post("/assistant", async (req, res) => {
  try {
    const { message, context, userId, history } = req.body;
    if (!message || typeof message !== "string" || message.length > 2e3) {
...
${postsSummary}`;
    ...
    const sanitizedHistory = Array.isArray(history) ? history.filter((m) => m && (m.role === "user" || m.role === "assistant") && typeof m.content === "string").slice(-10).map((m) => ({ role: m.role, content: m.content.slice(0, 1e3) })) : [];
    const zai = await getAI();
    if (!zai) {
...
${userInfo || "\u0645\u0644\u0627\u062D\u0638\u0629: \u0627\u0644\u0645\u0633\u062A\u062E\u062F\u0645 \u063A\u064A\u0631 \u0645\u0633\u062C\u0644 \u062F\u062E\u0648\u0644 - \u0642\u062F\u0645 \u0625\u062C\u0627\u0628\u0627\u062A \u0639\u0627\u0645\u0629."}`
    ...
    const aiReply = completion.choices?.[0]?.message?.content || generateSmartFallback(message, userInfo);
...
function generateSmartFallback(message, userInfo) {
...
\u0623\u0641\u0636\u0644 \u062E\u064A\u0627\u0631 \u0644\u0632\u064A\u0627\u062F\u0629 \u0648\u0635\u0648\u0644 \u0625\u0639\u0644\u0627\u0646\u0643 \u0628\u0634\u0643\u0644 \u0643\u0628\u064A\u0631! \u{1F680}`;
...
\u062E\u064A\u0627\u0631 \u0645\u0645\u062A\u0627\u0632 \u0644\u0632\u064A\u0627\u062F\u0629 \u0648\u0635\u0648\u0644 \u0625\u0639\u0644\u0627\u0646\u0643! \u{1F4C8}`;
...
\u0628\u062F\u0627\u064A\u0629 \u062C\u064A\u062F\u0629 \u0644\u0644\u062A\u0631\u0648\u064A\u062C! \u064A\u0645\u0643\u0646\u0643 \u0634\u062D\u0646 \u0645\u062D\u0641\u0638\u062A\u0643 \u0644\u0644\u0627\u0633\u062A\u0641\u0627\u062F\u0629 \u0645\u0646 \u0628\u0627\u0642\u0627\u062A \u0623\u0639\u0644\u0649. \u{1F4AA}`;
...
\u0627\u0628\u062F\u0623 \u062A\u062C\u0631\u0628\u062A\u0643 \u0645\u0639 \u0627\u0644\u062A\u0631\u0648\u064A\u062C! \u064A\u0645\u0643\u0646\u0643 \u0634\u062D\u0646 \u0645\u062D\u0641\u0638\u062A\u0643 \u0644\u0644\u0627\u0633\u062A\u0641\u0627\u062F\u0629 \u0645\u0646 \u0628\u0627\u0642\u0627\u062A \u0623\u0639\u0644\u0649 \u0644\u0627\u062D\u0642\u0627\u064B. \u2B50`;
    ...
    return `\u0623\u0642\u062A\u0631\u062D \u0628\u0627\u0642\u0629 "\u0645\u0645\u064A\u0632" - \u0627\u0644\u0623\u0643\u062B\u0631 \u0637\u0644\u0628\u0627\u064B! \u{1F31F}
...
\u0634\u062D\u0646 \u0645\u062D\u0641\u0638\u062A\u0643 \u0627\u0644\u0622\u0646 \u0644\u0644\u0627\u0633\u062A\u0641\u0627\u062F\u0629 \u0645\u0646 \u0628\u0627\u0642\u0629 \u0623\u0639\u0644\u0649! \u{1F4B0}`;
  }
  if (msg.includes("\u0633\u0639\u0631") || msg.includes("\u0643\u0645") || msg.includes("\u062A\u0643\u0644\u0641\u0629") || msg.includes("\u0628\u0643\u0627\u0645")) {
    return `\u{1F4B0} \u0623\u0633\u0639\u0627\u0631 \u0628\u0627\u0642\u0627\u062A \u0627\u0644\u062A\u0631\u0648\u064A\u062C:
...
\u0643\u0644\u0645\u0627 \u0632\u0627\u062F\u062A \u0627\u0644\u0628\u0627\u0642\u0629\u060C \u0632\u0627\u062F \u0627\u0644\u0648\u0635\u0648\u0644 \u0644\u0625\u0639\u0644\u0627\u0646\u0643! \u{1F4C8}`;
  }
  if (msg.includes("\u0643\u064A\u0641") || msg.includes("\u0637\u0631\u064A\u0642\u0629") || msg.includes("\u0634\u0631\u062D") || msg.includes("\u0627\u0632\u0627\u064A")) {
    return `\u{1F680} \u0627\u0644\u062A\u0631\u0648\u064A\u062C \u0639\u0644\u0649 \u0646\u0648\u0627\u0642\u0635 \u0633\u0647\u0644 \u062C\u062F\u0627\u064B!

1\uFE0F\u20E3 \u0627\u062E\u062A\u0631 \u0625\u0639\u0644\u0627\u0646\u0643 \u0645\u0646 \u0645\u0646\u0634\u0648\u0631\u0627\u062A\u0643
2\uFE0F\u20E3 \u0627\u0636\u063A\u0637 \u0632\u0631 "\u062A\u0631\u0648\u064A\u062C" \u{1F4E3}
3\uFE0F\u20E3 \u0627\u062E\u062A\u0631 \u0627\u0644\u0628\u0627\u0642\u0629 \u0627\u0644\u0645\u0646\u0627\u0633\u0628\u0629 \u0644\u0645\u064A\u0632\u0627\u0646\u064A\u062A\u0643
4\uFE0F\u20E3 \u0627\u062F\u0641\u0639 \u0645\u0646 \u0645\u062D\u0641\u0638\u062A\u0643 \u0627\u0644\u0625\u0644\u0643\u062A\u0631\u0648\u0646\u064A\u0629
5\uFE0F\u20E3 \u0627\u0646\u062A\u0638\u0631 \u0645\u0648\u0627\u0641\u0642\u0629 \u0627\u0644\u0625\u062F\u0627\u0631\u0629 (\u062F\u0642\u0627\u0626\u0642)
6\uFE0F\u20E3 \u064A\u0628\u062F\u0623 \u0627\u0644\u062A\u0631\u0648\u064A\u062C \u0641\u0648\u0631\u0627\u064B! \u{1F389}

\u0644\u0627 \u062A\u0646\u0633\u064E \u0634\u062D\u0646 \u0645\u062D\u0641\u0638\u062A\u0643 \u0623\u0648\u0644\u0627\u064B \u0645\u0646 \u0635\u0641\u062D\u0629 \u0627\u0644\u0645\u062D\u0641\u0638\u0629. \u{1F4B0}`;
  }
  if (msg.includes("\u062A\u062D\u0633\u064A\u0646") || msg.includes("\u0646\u0635\u064A\u062D\u0629") || msg.includes("\u0646\u0635\u0627\u0626\u062D") || msg.includes("\u0643\u064A\u0641 \u0627")) {
    return `\u{1F4A1} \u0646\u0635\u0627\u0626\u062D \u0644\u0632\u064A\u0627\u062F\u0629 \u0641\u0639\u0627\u0644\u064A\u0629 \u0627\u0644\u062A\u0631\u0648\u064A\u062C:

1. \u{1F4F8} \u0623\u0636\u0641 \u0635\u0648\u0631\u0629 \u0648\u0627\u0636\u062D\u0629 \u0639\u0627\u0644\u064A\u0629 \u0627\u0644\u062C\u0648\u062F\u0629 (\u062A\u0641\u0636\u0644 1200\xD7800)
2. \u270D\uFE0F \u0627\u0643\u062A\u0628 \u0639\u0646\u0648\u0627\u0646 \u062C\u0630\u0627\u0628 \u0648\u0623\u0648\u0644 50 \u062D\u0631\u0641 \u0647\u064A \u0627\u0644\u0623\u0647\u0645
3. \u{1F4B5} \u062D\u062F\u062F \u0627\u0644\u0633\u0639\u0631 \u0628\u0648\u0636\u0648\u062D \u0648\u0644\u0627 \u062A\u0628\u0627\u0644\u063A
4. \u{1F3AF} \u0627\u062E\u062A\u0631 \u0627\u0644\u0627\u0633\u062A\u0647\u062F\u0627\u0641 \u0627\u0644\u0645\u0646\u0627\u0633\u0628 (\u0645\u062F\u0646/\u0627\u0647\u062A\u0645\u0627\u0645\u0627\u062A)
5. \u{1F4AC} \u0631\u062F \u0639\u0644\u0649 \u0627\u0644\u062A\u0639\u0644\u064A\u0642\u0627\u062A \u0628\u0633\u0631\u0639\u0629
6. \u{1F504} \u062C\u062F\u062F \u0627\u0644\u062A\u0631\u0648\u064A\u062C \u0625\u0630\u0627 \u0644\u0627\u0642\u0649 \u062A\u0641\u0627\u0639\u0644\u0627\u064B \u062C\u064A\u062F\u0627\u064B
7. \u23F0 \u0623\u0641\u0636\u0644 \u0648\u0642\u062A \u0644\u0644\u062A\u0631\u0648\u064A\u062C: 7-10 \u0645\u0633\u0627\u0621\u064B

\u062D\u0638 \u0645\u0648\u0641\u0642! \u{1F340}`;
  }
  if (msg.includes("\u0648\u0635\u0648\u0644") || msg.includes("\u0645\u0634\u0627\u0647\u062F\u0627\u062A") || msg.includes("reach")) {
    return `\u{1F4CA} \u0627\u0644\u0648\u0635\u0648\u0644 \u064A\u0639\u0646\u064A \u0639\u062F\u062F \u0627\u0644\u0623\u0634\u062E\u0627\u0635 \u0627\u0644\u0630\u064A\u0646 \u0634\u0627\u0647\u062F\u0648\u0627 \u0625\u0639\u0644\u0627\u0646\u0643.
...
\u0643\u0644\u0645\u0627 \u0632\u0627\u062F \u0627\u0644\u0648\u0635\u0648\u0644\u060C \u0632\u0627\u062F \u0627\u062D\u062A\u0645\u0627\u0644 \u0627\u0644\u0628\u064A\u0639! \u{1F3AF}
\u0646\u0635\u064A\u062D\u0629: \u0631\u0627\u0642\u0628 "\u062A\u062D\u0644\u064A\u0644\u0627\u062A \u0627\u0644\u062A\u0631\u0648\u064A\u062C" \u0644\u0645\u0639\u0631\u0641\u0629 \u0623\u064A \u0627\u0644\u0628\u0627\u0642\u0627\u062A \u062A\u062C\u0644\u0628 \u0644\u0643 \u0623\u0641\u0636\u0644 \u0627\u0644\u0646\u062A\u0627\u0626\u062C.`;
  ...
  if (msg.includes("\u0645\u062D\u0641\u0638\u0629") || msg.includes("\u0631\u0635\u064A\u062F") || msg.includes("\u0634\u062D\u0646")) {
    return `\u{1F4B0} \u0627\u0644\u0645\u062D\u0641\u0638\u0629 \u0627\u0644\u0625\u0644\u0643\u062A\u0631\u0648\u0646\u064A\u0629:
...
\u0634\u062D\u0646 \u0645\u062D\u0641\u0638\u062A\u0643 \u0627\u0644\u0622\u0646 \u0644\u0644\u0627\u0633\u062A\u0641\u0627\u062F\u0629 \u0645\u0646 \u0628\u0627\u0642\u0627\u062A \u0627\u0644\u062A\u0631\u0648\u064A\u062C! \u{1F680}`;
  }
...
\u0644\u0644\u0633\u0631\u0639\u0629 \u0641\u064A \u0627\u0644\u0645\u0648\u0627\u0641\u0642\u0629\u060C \u062A\u0623\u0643\u062F \u0645\u0646 \u0623\u0646 \u0625\u0639\u0644\u0627\u0646\u0643: \u2705 \u0645\u062D\u062A\u0648\u0649 \u0648\u0627\u0636\u062D \u2705 \u0633\u0639\u0631 \u0645\u0646\u0627\u0633\u0628 \u2705 \u0635\u0648\u0631\u0629 \u062D\u0642\u064A\u0642\u064A\u0629`;
  ...
  if (msg.includes("\u0634\u0643\u0631\u0627") || msg.includes("\u0634\u0643\u0631\u0627\u064B") || msg.includes("\u062A\u0645\u0627\u0645") || msg.includes("ok")) {
    return `\u0627\u0644\u0639\u0641\u0648! \u{1F60A} \u0623\u0646\u0627 \u062F\u0627\u0626\u0645\u0627\u064B \u0647\u0646\u0627 \u0644\u0645\u0633\u0627\u0639\u062F\u062A\u0643. \u0625\u0630\u0627 \u0627\u062D\u062A\u062C\u062A \u0623\u064A \u0627\u0633\u062A\u0641\u0633\u0627\u0631 \u0622\u062E\u0631 \u0639\u0646 \u0627\u0644\u062A\u0631\u0648\u064A\u062C \u0623\u0648 \u0627\u0644\u0628\u0627\u0642\u0627\u062A\u060C \u0644\u0627 \u062A\u062A\u0631\u062F\u062F \u0641\u064A \u0627\u0644\u0633\u0624\u0627\u0644. \u0628\u0627\u0644\u062A\u0648\u0641\u064A\u0642 \u0641\u064A \u0625\u0639\u0644\u0627\u0646\u0627\u062A\u0643! \u{1F340}`;
  }
  if (msg.includes("\u0645\u0631\u062D\u0628\u0627") || msg.includes("\u0627\u0644\u0633\u0644\u0627\u0645") || msg.includes("\u0647\u0627\u064A") || msg.includes("hello")) {
    return `\u0623\u0647\u0644\u0627\u064B \u0648\u0633\u0647\u0644\u0627\u064B \u0628\u0643! \u{1F44B}\u{1F389}

\u0623\u0646\u0627 \u0645\u0633\u0627\u0639\u062F \u0627\u0644\u062A\u0631\u0648\u064A\u062C \u0627\u0644\u0630\u0643\u064A \u0641\u064A \u0645\u0646\u0635\u0629 \u0646\u0648\u0627\u0642\u0635. \u064A\u0645\u0643\u0646\u0646\u064A \u0645\u0633\u0627\u0639\u062F\u062A\u0643 \u0641\u064A:

\u2022 \u{1F4E6} \u0627\u062E\u062A\u064A\u0627\u0631 \u0627\u0644\u0628\u0627\u0642\u0629 \u0627\u0644\u0645\u0646\u0627\u0633\u0628\u0629 \u0644\u0645\u064A\u0632\u0627\u0646\u064A\u062A\u0643
\u2022 \u2728 \u062A\u062D\u0633\u064A\u0646 \u0645\u062D\u062A\u0648\u0649 \u0625\u0639\u0644\u0627\u0646\u0643
\u2022 \u{1F4A1} \u0646\u0635\u0627\u0626\u062D \u0644\u0632\u064A\u0627\u062F\u0629 \u0627\u0644\u0648\u0635\u0648\u0644
\u2022 \u{1F4CA} \u0641\u0647\u0645 \u0625\u062D\u0635\u0627\u0626\u064A\u0627\u062A \u0627\u0644\u062A\u0631\u0648\u064A\u062C
\u2022 \u2753 \u0623\u064A \u0633\u0624\u0627\u0644 \u0622\u062E\u0631 \u0639\u0646 \u0627\u0644\u062A\u0631\u0648\u064A\u062C

\u0643\u064A\u0641 \u0623\u0642\u062F\u0631 \u0623\u0633\u0627\u0639\u062F\u0643 \u0627\u0644\u064A\u0648\u0645\u061F \u{1F60A}`;
  }
  if (msg.includes("vip") || msg.includes("\u0641\u064A \u0627\u064A \u0628\u064A")) {
    return `\u{1F451} \u0628\u0627\u0642\u0629 VIP - \u0627\u0644\u0623\u0642\u0648\u0649 \u0639\u0644\u0649 \u0627\u0644\u0625\u0637\u0644\u0627\u0642!

\u{1F4B0} \u0627\u0644\u0633\u0639\u0631: 500 \u062C.\u0645
\u{1F4CA} \u0627\u0644\u0648\u0635\u0648\u0644: 25,000 \u0645\u0634\u0627\u0647\u062F\u0629
\u23F0 \u0627\u0644\u0645\u062F\u0629: 10 \u0623\u064A\u0627\u0645
\u{1F514} \u0627\u0644\u0625\u0634\u0639\u0627\u0631\u0627\u062A: 600 \u0625\u0634\u0639\u0627\u0631 \u0645\u0628\u0627\u0634\u0631
...
\u0623\u0646\u0635\u062D\u0643 \u0628\u0647\u0627 \u0625\u0630\u0627 \u0643\u0627\u0646 \u0644\u062F\u064A\u0643 \u0645\u0646\u062A\u062C \u062A\u0631\u064A\u062F \u0628\u064A\u0639\u0647 \u0628\u0633\u0631\u0639\u0629! \u{1F680}`;
  }
  return `\u{1F914} \u0644\u0645 \u0623\u0641\u0647\u0645 \u0633\u0624\u0627\u0644\u0643 \u062A\u0645\u0627\u0645\u0627\u064B. \u064A\u0645\u0643\u0646\u0646\u064A \u0645\u0633\u0627\u0639\u062F\u062A\u0643 \u0641\u064A:

\u2022 \u{1F4E6} \u0627\u062E\u062A\u064A\u0627\u0631 \u0628\u0627\u0642\u0629 \u0627\u0644\u062A\u0631\u0648\u064A\u062C \u0627\u0644\u0645\u0646\u0627\u0633\u0628\u0629
\u2022 \u{1F4B0} \u0645\u0639\u0631\u0641\u0629 \u0627\u0644\u0623\u0633\u0639\u0627\u0631 \u0648\u0627\u0644\u062A\u0643\u0627\u0644\u064A\u0641
\u2022 \u2728 \u062A\u062D\u0633\u064A\u0646 \u0645\u062D\u062A\u0648\u0649 \u0625\u0639\u0644\u0627\u0646\u0643
\u2022 \u{1F4A1} \u0646\u0635\u0627\u0626\u062D \u0644\u0632\u064A\u0627\u062F\u0629 \u0627\u0644\u0648\u0635\u0648\u0644
\u2022 \u{1F4CA} \u0634\u0631\u062D \u0625\u062D\u0635\u0627\u0626\u064A\u0627\u062A \u0627\u0644\u062A\u0631\u0648\u064A\u062C
\u2022 \u2753 \u0643\u064A\u0641 \u064A\u0639\u0645\u0644 \u0627\u0644\u062A\u0631\u0648\u064A\u062C \u062E\u0637\u0648\u0629 \u0628\u062E\u0637\u0648\u0629

\u0627\u0643\u062A\u0628 \u0633\u0624\u0627\u0644\u0643 \u0628\u0634\u0643\u0644 \u0645\u062E\u062A\u0644\u0641 \u0623\u0648 \u0627\u062E\u062A\u0631 \u0645\u0646 \u0627\u0644\u0645\u0648\u0627\u0636\u064A\u0639 \u0623\u0639\u0644\u0627\u0647. \u{1F60A}`;
}
...
var PERSISTENT_DIR4 = fs7.existsSync("/data") ? "/data" : path7.resolve(rootDir, "data");
console.log(`[SETUP] Persistent storage: ${PERSISTENT_DIR4}`);
var envPath = fs7.existsSync(path7.resolve(PERSISTENT_DIR4, ".env")) ? path7.resolve(PERSISTENT_DIR4, ".env") : path7.resolve(rootDir, ".env");
var envExamplePath = path7.resolve(rootDir, ".env.example");
if (!fs7.existsSync(envPath) && fs7.existsSync(envExamplePath)) {
  fs7.copyFileSync(envExamplePath, envPath);
  console.log(`[SETUP] Created .env at ${envPath}`);
}
for (const dir of ["uploads", "data", "backups"]) {
  const dirPath = path7.resolve(rootDir, dir);
...
for (const dir of ["uploads", "uploads/videos", "backups"]) {
  const dirPath = path7.resolve(PERSISTENT_DIR4, dir);
  if (!fs7.existsSync(dirPath)) {
    fs7.mkdirSync(dirPath, { recursive: true });
...
dotenv.config({ path: path7.resolve(rootDir, ".env.local") });
function autoSetEnv(key, value) {
  process.env[key] = value;
  try {
    const envContent = fs7.existsSync(envPath) ? fs7.readFileSync(envPath, "utf-8") : "";
    ...
    const persistentEnvPath = path7.resolve(PERSISTENT_DIR4, ".env");
    if (persistentEnvPath !== envPath) {
      try {
        const pEnvContent = fs7.existsSync(persistentEnvPath) ? fs7.readFileSync(persistentEnvPath, "utf-8") : "";
        ...
        fs7.writeFileSync(persistentEnvPath, pLines.join("\n"), "utf-8");
      } catch {
      }
    }
  } catch {
...
if (!jwtSecret || jwtSecret === "REPLACE-WITH-YOUR-OWN-SECURE-RANDOM-STRING") {
  const generatedSecret = crypto5.randomBytes(64).toString("hex");
  autoSetEnv("JWT_SECRET", generatedSecret);
  console.log("[CONFIG] JWT_SECRET auto-generated and saved to .env");
}
async function startServer() {
  try {
    const { autoRestoreDB: autoRestoreDB2 } = await Promise.resolve().then(() => (init_auto_restore(), auto_restore_exports));
    await autoRestoreDB2();
  } catch (err) {
    console.warn("[RESTORE] Failed:", err.message);
  }
  const app = express();
  ...
  app.use("/uploads", express.static(path7.resolve("uploads"), {
    maxAge: "7d",
    etag: true,
    lastModified: true
  }));
  const videosDir = path7.resolve("uploads/videos");
  if (!fs7.existsSync(videosDir)) fs7.mkdirSync(videosDir, { recursive: true });
  ...
  const downloadsDir = fs7.existsSync("/data/downloads") ? "/data/downloads" : path7.resolve(rootDir, "downloads");
  if (!fs7.existsSync(downloadsDir)) {
    fs7.mkdirSync(downloadsDir, { recursive: true });
  ...
  app.get("/get-app", (_req, res) => {
    const downloadPagePath = path7.resolve(rootDir, "public", "download.html");
    const distDownloadPath = path7.resolve(rootDir, "dist", "client", "download.html");
    if (fs7.existsSync(distDownloadPath)) {
      res.sendFile(distDownloadPath);
    } else if (fs7.existsSync(downloadPagePath)) {
  ...
  app.get("/install", (_req, res) => {
    const installPagePath = path7.resolve(rootDir, "public", "install.html");
    const distInstallPath = path7.resolve(rootDir, "dist", "client", "install.html");
    if (fs7.existsSync(distInstallPath)) {
      res.sendFile(distInstallPath);
    } else if (fs7.existsSync(installPagePath)) {
  ...
  app.get("/firebase-setup", (_req, res) => {
    const setupPagePath = path7.resolve(rootDir, "public", "firebase-setup.html");
    const distSetupPath = path7.resolve(rootDir, "dist", "client", "firebase-setup.html");
    if (fs7.existsSync(distSetupPath)) {
      res.sendFile(distSetupPath);
    } else if (fs7.existsSync(setupPagePath)) {
  ...
  app.get("/firebase-setup-interactive", (_req, res) => {
    const setupPagePath = path7.resolve(rootDir, "public", "firebase-setup-interactive.html");
    const distSetupPath = path7.resolve(rootDir, "dist", "client", "firebase-setup-interactive.html");
    if (fs7.existsSync(distSetupPath)) {
      res.sendFile(distSetupPath);
    } else if (fs7.existsSync(setupPagePath)) {
      res.sendFile(setupPagePath);
  ...
  app.post("/api/notifications/register-device", async (req, res) => {
    try {
      ...
      if (authHeader?.startsWith("Bearer ")) {
        try {
          const { verifyToken: verifyToken2 } = await Promise.resolve().then(() => (init_auth(), auth_exports));
          const payload = verifyToken2(authHeader.split(" ")[1]);
  ...
  if (isDev) {
    ...
    const clientPath = path7.resolve(rootDir, "dist", "client");
    const distPath = fs7.existsSync(clientPath) ? clientPath : path7.resolve(rootDir, "dist");
    ...
    app.get("*", (req, res, next) => {
      if (req.path.startsWith("/api/") || req.path.startsWith("/download/") || req.path.startsWith("/uploads/") || req.path === "/get-app" || req.path === "/health" || req.path === "/manifest.webmanifest" || req.path === "/sw.js" || req.path === "/offline.html" || req.path.startsWith("/icons/")) {
        return next();
      }
  ...
  server.listen(PORT, "0.0.0.0", async () => {
    ...
    try {
      const { initAutoBackup: initAutoBackup2 } = await Promise.resolve().then(() => (init_backup_system(), backup_system_exports));
      initAutoBackup2();
    } catch (err) {
