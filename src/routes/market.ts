...
router.post('/market-live/interact', authMiddleware, (req: Request, res: Response) => {
  try {
    const payload = (req as any).user as JwtPayload;
    ...
    const listing = db.prepare('SELECT * FROM market_listings WHERE id = ?').get(videoId) as any;
    if (!listing) { res.status(404).json({ error: 'الإعلان غير موجود' }); return; }

    switch (interactionType) {
      case 'like':
        // 🔧 FIX: was incrementing saves_count (wrong!) → now increments likes_count
        db.prepare("UPDATE market_listings SET likes_count = likes_count + 1, updated_at = datetime('now') WHERE id = ?").run(videoId);
        break;
      case 'save':
        // Toggle save
        const existing = db.prepare('SELECT id FROM market_listing_saves WHERE listing_id = ? AND user_id = ?').get(videoId, payload.userId) as any;
        if (!existing) {
          db.prepare('INSERT INTO market_listing_saves (listing_id, user_id) VALUES (?, ?)').run(videoId, payload.userId);
          db.prepare("UPDATE market_listings SET saves_count = saves_count + 1, updated_at = datetime('now') WHERE id = ?").run(videoId);
        }
        break;
      case 'share':
        // 🔧 FIX: was incrementing views_count (wrong!) → now increments shares_count
        db.prepare("UPDATE market_listings SET shares_count = shares_count + 1, updated_at = datetime('now') WHERE id = ?").run(videoId);
        break;
      case 'view':
        db.prepare("UPDATE market_listings SET views_count = views_count + 1, updated_at = datetime('now') WHERE id = ?").run(videoId);
        break;
    }
