...
class ApiClient {
  ...
  async aiAssistant(message: string, userId?: string) {
    return this.request<{ success: boolean; reply: string }>('/ai/assistant', {
      method: 'POST', body: JSON.stringify({ message, userId }),
    });
  }
