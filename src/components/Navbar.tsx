  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-1 flex-1 justify-center h-full">
        <button
          onClick={() => navigate('/')}
          className="h-full px-10 border-b-4 border-orange-600 text-orange-600 flex items-center"
        >
          <Home className="w-7 h-7" />
        </button>
        <button
          onClick={() => navigate('/market')}
          className={`h-full px-10 border-b-4 border-transparent ${textMuted} hover:bg-gray-50 hover:rounded-xl my-1 flex items-center transition-all group`}
        >
          <ShoppingBag className="w-7 h-7 group-hover:scale-110 group-hover:text-orange-600 transition-all" />
        </button>
      </div>
