...
export const VideoRecorder: React.FC<VideoRecorderProps> = ({ onClose, onLinked }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success(t('marketLive.videoLinked'));
      onLinked();
    } catch (err: any) {
          ...
          {mode === 'upload' && !videoFile && (
            <div>
              ...
              <button
                ...
                className={`w-full py-8 rounded-xl border-2 border-dashed flex flex-col items-center gap-3 transition-colors ${
                  darkMode
        ...
        <div className={`p-4 border-t ${darkMode ? 'border-gray-700' : 'border-gray-100'}`}>
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              disabled={!selectedAdId || !videoFile || uploading || linking}
