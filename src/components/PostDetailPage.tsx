...
export const PostDetailPage: React.FC = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto" dir={dir}>
        ...
        <div className={`p-12 text-center rounded-2xl border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
          ...
          <button onClick={() => navigate(-1)} className="mt-4 text-orange-600 font-bold text-sm hover:underline">{t('app.backToHome')}</button>
        </div>
      </div>
    );
  }
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className={`rounded-xl px-3 py-2 ${darkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
              ...
              {comment.image_url && (
                <img src={comment.image_url} alt="" className="mt-2 rounded-lg max-h-40 object-cover cursor-pointer" onClick={() => openImageModal(comment.image_url, 'Comment image')} />
              )}
            </div>
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 sm:px-4 pb-20" dir={dir}>
      {/* Header */}
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => navigate(-1)}
      ...
      <div className={`rounded-2xl border overflow-hidden mb-6 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
            ...
            <div>
              ...
              <div className={`flex items-center gap-1.5 text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                <span>{useMemo(() => {
                  ...
                  if (isNaN(date.getTime())) return post.timestamp;
                  const diffMin = Math.floor((now.getTime() - date.getTime()) / 60000);
                  if (diffMin < 1) return t('common.justNow', 'الآن');
            ...
            <AnimatePresence>
              {showMoreMenu && (
                <motion.div
                  ...
                  className={`absolute top-full ${dir === 'rtl' ? 'left-0' : 'right-0'} mt-1 w-48 rounded-xl shadow-xl border z-50 overflow-hidden ${
        ...
        {/* Promoted Badge on Detail */}
        {post.isPromoted && post.promotionStatus === 'approved' && (
          <div className={`mx-5 mb-3 flex items-center gap-2 px-3 py-2 rounded-xl border ${darkMode ? 'bg-gradient-to-l from-gray-700 via-gray-700/95 to-gray-700/90 border-gray-600' : 'bg-gradient-to-l from-orange-50 via-amber-50 to-orange-50/80 border-orange-200'}`}>
            <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full shadow-sm bg-gradient-to-l from-orange-500 to-orange-600`}>
              <Sparkles className="w-3 h-3 text-white" />
              <span className="text-[9px] font-black text-white">{t('postCard.aiPromoted')}</span>
            </div>
            {post.promotionTier && (
              <span className={`text-[9px] px-2 py-0.5 rounded-full font-bold ${darkMode ? 'bg-orange-900/30 text-orange-400' : 'bg-orange-100 text-orange-700'}`}>
                {post.promotionTier === 'vip' ? '💎 VIP' : post.promotionTier === 'premium' ? '👑 ' + t('postCard.premium') : '⚡ ' + t('postCard.standard')}
              </span>
            )}
            {post.reachCount && (
              <div className={`flex items-center gap-1 text-[9px] font-bold ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                <Eye className="w-3 h-3" />{post.reachCount.toLocaleString()}
              </div>
            )}
          </div>
        )}
        ...
        <div className="px-5 pb-4">
          ...
          {(post.feeling || post.activity) && (
            <div className={`mt-2 flex items-center gap-2 text-sm ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
              {post.feeling && (
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-orange-50 text-orange-700'}`}>
                  {post.feeling === 'happy' ? '😊' : post.feeling === 'sad' ? '😢' : post.feeling === 'excited' ? '🎉' : post.feeling === 'grateful' ? '🙏' : post.feeling === 'proud' ? '💪' : post.feeling === 'love' ? '❤️' : '😊'} {String(t(`feelings.${post.feeling}`, post.feeling))}
                </span>
              )}
              {post.activity && (
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold ${darkMode ? 'bg-gray-700 text-gray-300' : 'bg-blue-50 text-blue-700'}`}>
                  {post.activity === 'selling' ? '🛒' : post.activity === 'buying' ? '💰' : post.activity === 'working' ? '💼' : '🎮'} {String(t(`activities.${post.activity}`, post.activity))}
                </span>
              )}
            </div>
          )}
        ...
        {/* Category & Condition */}
        {(post.category || (post as any).condition) && (
          <div className="px-5 pb-3 flex items-center gap-1.5 flex-wrap">
            {post.category && (
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${darkMode ? 'bg-gray-700 text-gray-300 border-gray-600' : 'bg-orange-50 text-orange-700 border-orange-100'}`}>
                <Tag className="w-2.5 h-2.5" /> {String(t(`categories.${post.category}`, post.category))}
              </span>
            )}
            {(post as any).condition && (
              <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${
                (post as any).condition === 'new' ? (darkMode ? 'bg-green-900/30 text-green-400 border-green-800' : 'bg-green-50 text-green-700 border-green-200')
                : (post as any).condition === 'like_new' ? (darkMode ? 'bg-emerald-900/30 text-emerald-400 border-emerald-800' : 'bg-emerald-50 text-emerald-700 border-emerald-200')
                : (post as any).condition === 'refurbished' ? (darkMode ? 'bg-blue-900/30 text-blue-400 border-blue-800' : 'bg-blue-50 text-blue-700 border-blue-200')
                : (darkMode ? 'bg-yellow-900/30 text-yellow-400 border-yellow-800' : 'bg-yellow-50 text-yellow-700 border-yellow-200')
              }`}>
                <Package className="w-2.5 h-2.5" /> {String(t(`common.${(post as any).condition === 'like_new' ? 'likeNew' : (post as any).condition}`, (post as any).condition))}
              </span>
            )}
          </div>
        )}
        ...
        {post.image && (
          <div
            className={`border-y cursor-pointer ${darkMode ? 'bg-gray-700 border-gray-700' : 'bg-gray-50 border-gray-100'}`}
            onClick={() => openImageModal(post.image!, 'Post content')}
          >
          ...
          <div className={`mx-5 my-4 p-5 rounded-2xl border ${darkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-100'}`}>
            ...
            {post.paymentMethods && post.paymentMethods.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap mb-4">
                ...
                {post.paymentMethods.map(method => {
                  const methodConfig: Record<string, { label: string; color: string; darkColor: string; icon: string }> = {
                    vf_cash: { label: t('postCard.vodafoneCash'), color: 'bg-red-50 text-red-600 border-red-100', darkColor: 'bg-red-900/30 text-red-400 border-red-800', icon: '📱' },
                    vodafone_cash: { label: t('postCard.vodafoneCash'), color: 'bg-red-50 text-red-600 border-red-100', darkColor: 'bg-red-900/30 text-red-400 border-red-800', icon: '📱' },
                    instapay: { label: t('postCard.instaPay'), color: 'bg-blue-50 text-blue-600 border-blue-100', darkColor: 'bg-blue-900/30 text-blue-400 border-blue-800', icon: '💳' },
                    cash: { label: t('postCard.cash', 'كاش'), color: 'bg-green-50 text-green-600 border-green-100', darkColor: 'bg-green-900/30 text-green-400 border-green-800', icon: '💵' },
                    bank_transfer: { label: t('postCard.bankTransfer', 'تحويل بنكي'), color: 'bg-purple-50 text-purple-600 border-purple-100', darkColor: 'bg-purple-900/30 text-purple-400 border-purple-800', icon: '🏦' },
                    fawry: { label: t('postCard.fawry', 'فوري'), color: 'bg-orange-50 text-orange-600 border-orange-100', darkColor: 'bg-orange-900/30 text-orange-400 border-orange-800', icon: '🏧' },
                  };
                  const cfg = methodConfig[method];
                  if (!cfg) return null;
                  return (
                    <div key={method} className={`px-2.5 py-1 rounded-lg text-[10px] font-black border ${darkMode ? cfg.darkColor : cfg.color}`}>
                      {cfg.icon} {cfg.label}
                    </div>
                  );
                })}
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex-1 bg-orange-600 hover:bg-orange-700 text-white py-3.5 rounded-xl text-sm font-bold shadow-lg shadow-orange-100 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
              >
              ...
              {((post as any).phone || (post as any).whatsapp) && (
                <button
                  ...
                  className={`p-3.5 rounded-xl transition-all active:scale-95 border ${darkMode ? 'bg-green-900/30 border-green-800 text-green-400 hover:bg-green-900/50' : 'bg-green-50 border-green-200 text-green-700 hover:bg-green-100'}`}
                  title="WhatsApp"
                >
        ...
        <div className={`px-5 py-3 flex items-center justify-between text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
          <div className="flex items-center gap-2">
            ...
            {(post as any).viewsCount > 0 && (
              <span className="flex items-center gap-0.5"><Eye className="w-3 h-3" />{(post as any).viewsCount}</span>
            )}
          </div>
          <div className="flex items-center gap-3">
      ...
      <div className={`rounded-2xl border overflow-hidden mb-6 ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
        ...
        <form onSubmit={handleAddComment} className={`flex items-center gap-2 px-3 sm:px-5 py-3 border-t ${darkMode ? 'border-gray-700' : 'border-gray-100'}`}>
          ...
          <button
            ...
            className={`p-2.5 rounded-xl transition-colors ${darkMode ? 'hover:bg-gray-700 text-gray-400 hover:text-orange-400' : 'hover:bg-gray-100 text-gray-400 hover:text-orange-600'}`}
          >
