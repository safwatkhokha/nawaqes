  ...
  const handleSubmit = async () => {
    ...
    try {
      const savedPost = await api.createPost({
        ...
        category: postType === 'ad' && savedCategory ? savedCategory : '',
    ...
    <div dir={dir} className={`${isModal ? 'pb-4' : `${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'} rounded-xl shadow-sm border p-4 mb-4`}`}>
        ...
        <div className="fixed inset-0 z-[300] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowStoryCreator(false)}>
          <div dir={dir} className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden`} onClick={e => e.stopPropagation()}>
            ...
            <div className={`p-4 space-y-3 ${darkMode ? 'bg-gray-800' : ''}`}>
              ...
              <input ref={storyImageRef} type="file" accept="image/*" className="hidden" onChange={(e) => {
                const file = e.target.files?.[0];
                if (file) { const reader = new FileReader(); reader.onload = (ev) => setStoryImage(ev.target?.result as string); reader.readAsDataURL(file); }
              }} />
              <button onClick={() => storyImageRef.current?.click()} className={`w-full flex items-center justify-center gap-2 p-3 ${darkMode ? 'bg-gray-700 hover:bg-gray-600' : 'bg-gray-50 hover:bg-gray-100'} rounded-xl transition-colors`}>
        ...
        <div className={`${isModal ? 'px-4 pt-2' : `mb-4 border ${darkMode ? 'border-gray-700' : 'border-gray-100'} rounded-2xl p-4`}`}>
          ...
          {!selectedImage && (
            <button
              ...
              className={`w-full mb-3 flex flex-col items-center justify-center gap-2 p-4 border-2 border-dashed rounded-xl transition-all ${
                darkMode
          ...
          {/* Category Selector for Ads — Scrollable horizontal chips */}
          {postType === 'ad' && (
            <div className="mb-3">
              <p className={`text-[10px] font-black uppercase tracking-wider mb-2 ${darkMode ? 'text-gray-400' : 'text-gray-400'}`}>{t('createPost.category')}</p>
              <div
      ...
      {!isOpen && !isModal && (
        <div className={`border-t ${darkMode ? 'border-gray-700' : 'border-gray-100'} pt-3 flex items-center justify-between`}>
          ...
          <button onClick={handleLiveVideo} className={`flex-1 flex items-center justify-center gap-2 py-2 ${darkMode ? 'hover:bg-red-900/30' : 'hover:bg-red-50'} rounded-lg transition-colors group`}>
            <Video className="w-5 h-5 text-red-500" />
            <span className={`text-sm font-semibold ${darkMode ? 'text-gray-300 group-hover:text-red-400' : 'text-gray-600 group-hover:text-red-700'}`}>{t('createPost.liveStreamVideo')}</span>
          </button>
