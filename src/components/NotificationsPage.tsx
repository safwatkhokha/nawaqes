...
type FilterType = 'all' | 'system' | 'promotion' | 'payment' | 'message' | 'match' | 'alert' | 'friend' | 'like' | 'comment' | 'share' | 'market' | 'warning';
...
export const NotificationsPage: React.FC = () => {
  ...
  const { notifications, readNotificationIds, markAllNotificationsRead, markNotificationRead, deleteNotification, darkMode, adminAlerts } = useAppContext();
  ...
  React.useEffect(() => {
    ...
    if (filterParam && ['all', 'system', 'promotion', 'payment', 'message', 'match', 'alert', 'friend', 'like', 'comment', 'share', 'market', 'warning'].includes(filterParam)) {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      if (Array.isArray(notifs)) {
        ...
        // Use the context's setNotifications indirectly by refreshing data
            ...
            <div className="space-y-2">
              {groupedNotifications.map((grouped, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Icon */}
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${getNotifIconBg(notif.type)}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        ...
                        {/* Navigation hint */}
                        {(notif.postId || notif.userId || notif.link) && (
