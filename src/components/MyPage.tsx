...
export const MyPage: React.FC = () => {
  ...
  const { darkMode, posts, savedPosts, toggleSavePost, openShareModal, sendMessage, addPost, acceptFriendRequest, rejectFriendRequest, isUserOnlineWs } = useAppContext();
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center justify-between mt-3">
              <div className="flex items-center gap-2">
                <label htmlFor="fileInputRef-input" className={`p-2 rounded-lg transition-colors ${selectedImage ? (darkMode ? 'bg-orange-900/30 text-orange-400' : 'bg-orange-100 text-orange-600') : (darkMode ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-500')}`} style={{cursor:"pointer"}}>
      ...
      <AnimatePresence mode="wait">
        ...
        {activeTab === 'friends' && (
          ...
          >
            ...
            {friendsList.filter((f: any) => isUserOnlineWs(f.id) || f.isOnline === true || isUserOnline(f.id)).length > 0 && (
              <div className={`rounded-2xl border p-4 ${cardBg}`}>
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-2.5 h-2.5 bg-green-500 rounded-full animate-pulse" />
                  <span className={`text-xs font-bold ${textSecondary}`}>{t('friends.onlineFriends')}</span>
                  <span className={`text-[10px] ${textMuted}`}>({friendsList.filter((f: any) => isUserOnlineWs(f.id) || f.isOnline === true || isUserOnline(f.id)).length})</span>
                </div>
                <div className="flex items-center gap-3 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
                  {friendsList.filter((f: any) => isUserOnlineWs(f.id) || f.isOnline === true || isUserOnline(f.id)).map((friend: any) => (
                    <button
                      key={friend.id}
                ...
                {loadingFriends ? (
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                    {filteredFriends.map((friend: any) => {
                      const online = friend.isOnline === true || isUserOnlineWs(friend.id) || isUserOnline(friend.id);
                      return (
                        <motion.div
                          key={friend.id}
                ...
                {loadingFriends ? (
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                    {friendRequests.map((request: any) => {
                      ...
                      const online = requester.isOnline === true || isUserOnlineWs(requester.id) || isUserOnline(requester.id);
                      return (
                        <motion.div
                ...
                {loadingFriends ? (
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto" style={{ scrollbarWidth: 'thin' }}>
                    {friendSuggestions.map((suggestion: any) => {
                      const online = suggestion.isOnline === true || isUserOnlineWs(suggestion.id) || isUserOnline(suggestion.id);
