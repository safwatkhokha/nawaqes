...
if (userCount.count === 0) {
  // Read admin credentials from environment variables (SECURE - no hardcoded passwords)
  const adminEmail = process.env.ADMIN_EMAIL || 'admin@nawaqes.com';
  let adminPassword = process.env.ADMIN_PASSWORD;
  if (!adminPassword) {
    adminPassword = 'Nawaqes@' + crypto.randomBytes(8).toString('hex');
...
try {
  ...
  if (!existingOwner) {
    if (!ownerPassword) {
      ownerPassword = 'Owner@' + crypto.randomBytes(8).toString('hex');
