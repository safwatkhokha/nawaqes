...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [darkMode, setDarkMode] = useState(() => localStorage.getItem('nawaqes_darkmode') === 'true');
  ...
  useEffect(() => {
    ...
    }, wsConnected ? 60000 : 15000); // Poll every 60s when WS connected, 15s when not
  ...
  const enableSmartAlerts = async (): Promise<boolean> => {
    try {
      ...
      if (permission === 'granted') {
        ...
        toast.info(t('settings.notificationsDenied'));
        return false;
      }
    } catch (err) {
